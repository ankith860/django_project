...
class PostListView(ListView): #View for an object we will be looping over to display, this must be provided
    model = Post #Variable that must be set so the class-view knows what model object to query/loop over, per django conventions
...
class UserPostListView(ListView): #View for an object we will be looping over to display, this must be provided
    ...
    def get_queryset(self): #modify/override query_set that this view returns
        ...
        return Post.objects.filter(author=user).order_by('-date_posted') #ordering was also overriden, so we add proper ordering in return here
